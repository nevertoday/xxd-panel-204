...
# XXD Panel 204｜مشهد ذاكرة ضوئية ناعم التركيز
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 204 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-204`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-204 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-204-prompt.en.md) أو [بالصينية](references/xxd-panel-204-prompt.zh-CN.md).
